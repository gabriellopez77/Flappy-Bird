...
Start_screen::Start_screen() {
	...
	getReady_image.position = glm::vec2(SCREEN_SIZE_HALF.x - getReady_image.size.x / 2.f, 100.f);
	getReady_image.setNormalizedTex(295, 59, 92, 25);

	clickToStart_image.size = glm::vec2(57 * 4, 49 * 4);
	clickToStart_image.position = glm::vec2(SCREEN_SIZE_HALF.x - clickToStart_image.size.x / 2.f, 350.);
